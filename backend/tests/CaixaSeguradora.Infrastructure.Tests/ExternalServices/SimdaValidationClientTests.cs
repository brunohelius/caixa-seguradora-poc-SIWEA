...
/// <summary>
/// Unit tests for SimdaValidationClient covering:
/// - T029: SOAP envelope construction and parsing
/// - Contract number validation (NUM_CONTRATO = 0 or null routing)
...
/// </summary>
public class SimdaValidationClientTests
{
    private readonly Mock<ILogger<SimdaValidationClient>> _mockLogger;
    private readonly Mock<IHttpClientFactory> _mockHttpClientFactory;

    public SimdaValidationClientTests()
    {
        _mockLogger = new Mock<ILogger<SimdaValidationClient>>();
        _mockHttpClientFactory = new Mock<IHttpClientFactory>();
    }

    #region T029: SOAP Success Scenarios

    [Fact]
    public async Task ValidateAsync_WithValidHbContract_ReturnsSuccessResponse()
    {
        // Arrange
        ...
        {
            BaseAddress = new Uri("https://simda.caixaseguradora.com.br/soap/")
        };

        _mockHttpClientFactory
            .Setup(f => f.CreateClient("SIMDA"))
            .Returns(httpClient);

        var client = new SimdaValidationClient(_mockHttpClientFactory.Object, _mockLogger.Object);

        var request = CreateValidRequest(numContrato: 0); // HB contract
    ...
    public async Task ValidateAsync_BuildsSoapEnvelopeCorrectly()
    {
        ...
        var request = CreateValidRequest(
            numContrato: null, // HB contract (no contract number)
            orgsin: 1,
            rmosin: 2,
            numsin: 123456,
            tipoPagamento: 3,
    ...
    [Theory]
    [InlineData(0, true)]       // HB contract (explicit zero)
    [InlineData(null, true)]    // HB contract (no contract number)
    [InlineData(1, false)]      // EFP contract (SIPUA territory)
    [InlineData(123456, false)] // EFP contract (SIPUA territory)
    ...
    [Fact]
    public async Task ValidateAsync_WithPositiveContract_ReturnsErrorResponse()
    {
        // Arrange
        var client = new SimdaValidationClient(_mockHttpClientFactory.Object, _mockLogger.Object);
        var request = CreateValidRequest(numContrato: 123456); // SIPUA territory
        ...
        Assert.Contains("not suitable for SIMDA", response.ErrorMessage);
        Assert.Equal("SIMDA", response.ValidationService);
    }
    ...
    public async Task ValidateAsync_WithNegativeContract_ReturnsErrorResponse()
    {
        ...
        Assert.False(response.IsSuccess);
        Assert.Equal("INVALID_CONTRACT", response.Ezert8);
        Assert.Contains("expected NUM_CONTRATO = 0 or null", response.ErrorMessage);
        Assert.Equal("SIMDA", response.ValidationService);
    }

    #endregion

    #region T029: Retry and Timeout Scenarios
    ...
    public async Task ValidateAsync_WithTransientFailure_RetriesThreeTimes()
    {
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("SIMDA retry")),
                It.IsAny<Exception>(),
    ...
    public async Task ValidateAsync_WithPersistentTimeout_ReturnsTimeoutError()
    {
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("SIMDA")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.AtLeastOnce());
    }
    ...
    #region T029: Circuit Breaker Tests
...
<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
  <soap:Body>
    <soap:Fault>
      ...
      <faultstring>Internal SIMDA error</faultstring>
    </soap:Fault>
  </soap:Body>
</soap:Envelope>";
...
<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:sim=""http://caixaseguradora.com.br/simda"">
  <soap:Body>
    <sim:ValidateHBContractResponse>
      <sim:Status>OK</sim:Status>
    </sim:ValidateHBContractResponse>
  </soap:Body>
</soap:Envelope>";
...
<soap:Envelope xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:sim=""http://caixaseguradora.com.br/simda"">
  <soap:Body>
    <sim:ValidateHBContractResponse>
      <sim:Ezert8>{ezert8Code}</sim:Ezert8>
      <sim:Message>{message}</sim:Message>
      <sim:Timestamp>{DateTime.UtcNow:O}</sim:Timestamp>
    </sim:ValidateHBContractResponse>
  </soap:Body>
</soap:Envelope>";
